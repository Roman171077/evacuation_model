...
2. Локальный поток на участке строится как **непрерывная группа из 2+ человек** (после сортировки по `x`), где продольный разрыв между соседями `<= 0.25 м`; одиночный человек помечается как «вне потока» (для него принимается `Di(t)=0`, т.е. свободное движение).
