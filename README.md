...
1. Плотность участка `D_vj(t)` вычисляется **без `dt`**:
   `D_vj(t) = (Σ f_i) / (a_j * b_j)`, где `Σ f_i` — сумма площадей всех людей на участке.
2. Локальный поток на участке строится как **непрерывная группа из 2+ человек** (после сортировки по `x`), где продольный разрыв между соседями `<= 0.5 м`; одиночный человек помечается как «вне потока» (для него принимается `Di(t)=0`, т.е. свободное движение).
